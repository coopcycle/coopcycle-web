...
const formatSlots = (
  ...
  const formattedSlots = {} as Record<string, string[]>;
...
const TimeSlotPicker = ({ storeNodeId, taskId, timeSlotLabels }: Props) => {
  ...
  const { data: timeSlotChoicesResponse, isFetching: isLoadingChoices } =
    useGetTimeSlotChoicesQuery(taskValues.timeSlotUrl, {
  ...
  const setTimeSlotUrl = useCallback(
    (timeSlotUrl: string) => {
  ...
  const setTimeSlotFromDateAndRange = useCallback(
    (date, hourRange) => {
      const [first, second] = hourRange.split('-');
      const startTime = date
        .clone()
        .hours(first.split(':')[0])
        .minutes(first.split(':')[1]);
      const endTime = date
        .clone()
        .hours(second.split(':')[0])
        .minutes(second.split(':')[1]);
  ...
  useEffect(() => {
    ...
    storeTimeSlotIds,
    storeDefaultTimeSlotId,
    taskValues.timeSlotUrl,
    setTimeSlotUrl,
    mode,
  ]);
  ...
  useEffect(() => {
    ...
    if (timeSlotChoicesResponse['choices'].length === 0) {
      ...
      setTimeSlotFromDateAndRange(
        firstDate,
        formattedSlots[availableDates[0]][0],
      );
    }
  }, [
    timeSlotChoicesResponse,
    setTimeSlotFromDateAndRange,
    setTimeSlotUrl,
    setTimeSlot,
  ]);
  ...
  const availableDates = Object.keys(formattedTimeslots || {}).map(date =>
    moment(date),
  );
  ...
  const selectedDate = extractDateAndRangeFromTimeSlot(
    taskValues.timeSlot,
  ).date;
  const selectedHour = extractDateAndRangeFromTimeSlot(
    taskValues.timeSlot,
  ).hour;

  const hourOptions =
    formattedTimeslots[selectedDate.format('YYYY-MM-DD')] || [];
