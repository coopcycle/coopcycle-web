...
function mapStateToProps(state) {
  ...
  if (hasError) {
    const errorCodes = state.errors.shippingAddress.map(error => error.code)

    isAddressTooFar = _.includes(errorCodes, ADDRESS_TOO_FAR)

    isError = isAddressTooFar || _.includes(errorCodes, ADDRESS_NOT_PRECISE)
