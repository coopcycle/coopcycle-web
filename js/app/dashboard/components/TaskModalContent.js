...
class TaskModalContent extends React.Component {
  ...
  onCancelClick(task) {
    if (
      window.confirm(
        this.props.t('ADMIN_DASHBOARD_CANCEL_TASK_CONFIRM', { id: task.id }),
      )
    ) {
  ...
  onRestoreClick(task) {
    if (
      window.confirm(
        this.props.t('ADMIN_DASHBOARD_RESTORE_TASK_CONFIRM', { id: task.id }),
      )
    ) {
  ...
  _validate(values) {
    ...
    if (
      ...
      values.address.telephone.trim().length > 0
    ) {
  ...
  _onSubmit(values) {
    ...
    if (
      ...
      Object.prototype.hasOwnProperty.call(task.address, '@id')
    ) {
      values = {
        ...values,
        address: {
      ...
      <div className="modal-footer">
        {!!task && task.status === 'TODO' && (
          <button
            ...
            <span>{this.props.t('ADMIN_DASHBOARD_CANCEL_TASK')}</span>
          </button>
        )}
        {!!task && task.status === 'CANCELLED' && (
          <button
            ...
            <span>{this.props.t('ADMIN_DASHBOARD_RESTORE')}</span>
          </button>
        )}
        {!!task && Object.prototype.hasOwnProperty.call(task, '@id') && (
          <button
            ...
            <span className="text-success">
              {this.props.t('ADMIN_DASHBOARD_DUPLICATE_TASK')}
            </span>
          </button>
        )}
        {!!task &&
          (task.status === 'CANCELLED' || task.status === 'FAILED') && (
            <button
              ...
              <span>{this.props.t('ADMIN_DASHBOARD_RESCHEDULE')}</span>
            </button>
          )}
        {!!task &&
          task.isAssigned &&
                ...
                {task.status === 'TODO' && (
                  <li>
                    <a href="#" onClick={e => this.onStartClick(task, e)}>
                      ...
                      <span>{this.props.t('ADMIN_DASHBOARD_START_TASK')}</span>
                    </a>
                  </li>
                )}
                <li>
      ...
      <div>
        <div className="text-center">
          <a
            ...
            {...anchorProps}>
            <small>
              {this.props.t('ADMIN_DASHBOARD_TASK_FORM_SHOW_HISTORY')}
            </small>
          </a>
        </div>
      ...
      <div>
        <div className="text-center">
          <a
            ...
            {...anchorProps}>
            <small>
              {this.props.t('ADMIN_DASHBOARD_TASK_FORM_SHOW_BARCODE')}
            </small>
          </a>
        </div>
  ...
  renderForm() {
    let initialValues = {
      ...
      after: moment(this.props.date)
        .set({ hour: 0, minute: 0, second: 0 })
        .format(),
      before: moment(this.props.date)
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    completeTask: (task, notes, success) =>
      dispatch(completeTask(task, notes, success)),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withTranslation()(TaskModalContent))
