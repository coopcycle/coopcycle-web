...
trait RestaurantTrait
{
    ...
    protected function renderRestaurantDashboard(Request $request, Restaurant $restaurant, OrderInterface $order = null)
    {
        ...
        return $this->render($request->attributes->get('template'), $this->withRoutes([
            ...
            'orders' => $orders,
            'order' => $order,
            'orders_normalized' => $this->get('serializer')->normalize($orders, null, ['groups' => ['order', 'order_timeline']]),
            'order_normalized' => $this->get('serializer')->normalize($order, null, ['groups' => ['order', 'order_timeline']]),
            'routes' => $routes,
            'date' => $date,
        ], $routes));
    }
