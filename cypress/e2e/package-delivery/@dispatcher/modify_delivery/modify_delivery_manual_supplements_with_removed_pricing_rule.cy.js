context('Delivery (role: dispatcher)', () => {
  ...
  it('modify order', function () {
    ...
    cy.get('[data-testid="manual-supplement-range-Priority handling"]').within(
      () => {
        cy.get('[data-testid="range-input-field"]').type('{selectall}4');
      },
    );
    ...
    cy.get('[data-testid="order-item-0"]').within(() => {
      cy.get('[data-testid="adjustment-1"]').within(() => {
        ...
        cy.get('[data-testid="price"]').should('contain', '€4.00');
      });
    });
