Feature: Shift activities
  ...
  Scenario: The default activities are available out of the box
    ...
    And the JSON should match:
      """
      {
        ...
        "hydra:member":[
          { "@id":"/api/shift_activities/1", "@type":"ShiftActivity", "id":1, "slug":"delivery", "label":"Delivery", "color":null, "addToDispatch":true },
          { "@id":"/api/shift_activities/2", "@type":"ShiftActivity", "id":2, "slug":"dispatch", "label":"Dispatch", "color":null, "addToDispatch":false },
          { "@id":"/api/shift_activities/3", "@type":"ShiftActivity", "id":3, "slug":"administration", "label":"Administration", "color":null, "addToDispatch":false }
        ],
        "hydra:totalItems":3
      }
      """
  ...
  Scenario: Dispatcher toggles whether an activity is added to the dispatch
    Given the user "bob" is loaded:
      | email    | bob@coopcycle.org |
      | password | 123456            |
    And the user "bob" has role "ROLE_DISPATCHER"
    ...
    When I add "Content-Type" header equal to "application/ld+json"
    And I add "Accept" header equal to "application/ld+json"
    And the user "bob" sends a "PUT" request to "/api/shift_activities/2" with body:
      """
      {
        "label": "Dispatch",
        "addToDispatch": true
