...
final class Version20200503164147 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $stmts['orders_from_user'] =
            $this->connection->prepare('SELECT * FROM sylius_order WHERE customer_id = :customer_id');
        ...
        while ($user = $stmts['users']->fetch()) {
            ...
            $stmts['orders_from_user']->bindParam('customer_id', $user['id']);
            $stmts['orders_from_user']->execute();

            while ($order = $stmts['orders_from_user']->fetch()) {
                $this->addSql('UPDATE sylius_order SET customer_id = (SELECT id FROM sylius_customer WHERE email_canonical = :email_canonical) WHERE id = :order_id', [
                    'email_canonical' => $user['email_canonical'],
                    'order_id' => $order['id'],
                ]);
            }
